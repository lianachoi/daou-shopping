...
function SearchOrder() {
  ...
  const getOrder = (e) => {
    Axios.get("/api/v1/orders/" + order.orderId)
      .then((response) => {
    ...
    <>
      ...
      {order.userName !== "" ? (
        <>
          <div
            className="order-box"
            style={{ height: 200 + 100 * itemList.length + "px" }}
          >
            결제 일자: {moment(order.orderDate).format("YYYY-MM-DD")}
            <br />
            <br />
            <br />
            <div>
          ...
          >
            ...
            {order.useCard ? (
              <>
                ...
                <div>
                  결제 금액:{" "}
                  {addComma(total - order.usePoint - order.useCoupon)}
                </div>
              </>
            ) : (
              <>
